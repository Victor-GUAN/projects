...
#        X_embedded = p.reshape(8, 2)
#        K=[50,100,150,200,250]
#        if i in K:
#            print "ite = " , i,  momentum,'X =', X_embedded 
...
class TSNE(BaseEstimator):
    ...
    def _tsne(self, P, degrees_of_freedom, n_samples, random_state,
        ...
        B=[[]]
        B[0] = set_1_X[1].reshape(n_samples, self.n_components)
        set_X_embedded.extend(B)
        C=[[]]
        C[0] = set_2_X[0].reshape(n_samples, self.n_components)
        ...
        return X_embedded, set_X_embedded
...
model = TSNE(n_components=2, perplexity=5, learning_rate=100.0, n_iter=400, random_state=0)
np.set_printoptions(suppress=True)
Y, set_TSNE = model.fit_transform(Z)
