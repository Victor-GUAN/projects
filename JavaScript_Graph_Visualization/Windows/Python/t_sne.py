...
#       collect the array p in a set - set_p
 
#       note the first element of the set_p for initialization       
        if (i + 1) % step == 0 :
            if i + 1 == it - it % step + step :
                set_p[0] = copy.deepcopy(p)
                
###     N.B!
###     here, A and set_p are lists, and p is a dynamic variable, so we can't simlp use "=p",
###     we must consider the problem of pointer and adress, so we use the function deepcopy() to fix the value.                
...
class TSNE(BaseEstimator):
    ...
    def __init__(self, n_components=2, perplexity=30.0,
                 ...
                 random_state=None, method='barnes_hut', angle=0.5,step=50):
    ...
    def _fit(self, X, skip_num_points=0):
        ...
        if self.step > self.n_iter:
            raise ValueError("step should not be greater than n_iter")
        ...
        if isinstance(self.init, np.ndarray):
            ...
            X_embedded = 1e-8 * pca.fit_transform(X)
    ...
    def _tsne(self, P, degrees_of_freedom, n_samples, random_state,
		...
		# first 50 iterations with the parametres initial
        params, kl_divergence, it, set_1_X = _gradient_descent(obj_func, params,
                                                      **opt_args)

        opt_args['n_iter'] = 100
        opt_args['momentum'] = 0.8
        opt_args['it'] = it + 1
		# second 50 ~ 100 iterations with paranetres definied
        params, kl_divergence, it, set_2_X = _gradient_descent(obj_func, params,
                                                      **opt_args)
        ...
        opt_args['it'] = it + 1
		# the last iterations
        params, error, it, set_3_X = _gradient_descent(obj_func, params,
                                                      **opt_args)
        print error
        print it, opt_args['n_iter']
...
#       creat the list of embedding vectors in each step
#       different conditions bwteen the number of step and the amount of samples with the consideration of the first 50 iter and the second 50 ~ 100 iter.

        if self.step <= 100 :       
            if self.step <=50 :
                set_X_embedded[0] = set_1_X[0].reshape(n_samples, self.n_components)
                if self.step <=25 :                       
                    for i in range(1, len(set_1_X)):
                        B=[[]]
                        B[0] = set_1_X[i].reshape(n_samples, self.n_components)
                        set_X_embedded.extend(B)                
                for i in range(len(set_2_X)):
                    C=[[]]
                    C[0] = set_2_X[i].reshape(n_samples, self.n_components)
                    set_X_embedded.extend(C)
            else :
                set_X_embedded[0] = set_2_X[0].reshape(n_samples, self.n_components)
                                      
            for i in range(len(set_3_X)):
                D=[[]]
                D[0] = set_3_X[i].reshape(n_samples, self.n_components)
                set_X_embedded.extend(D)
                
        if self.step > 100 :
            set_X_embedded[0] = set_3_X[0].reshape(n_samples, self.n_components)
            if self.step * 2 <= self.n_iter :
                for i in range(1, len(set_3_X)):
                    D=[[]]
                    D[0] = set_3_X[i].reshape(n_samples, self.n_components)
                    set_X_embedded.extend(D)                
...
#       creat the dictionary of embedding vectors in each step
        
        Dict_X_embedded = {}
        for j in range (1, self.n_iter / self.step + 1):
            Dict_X_embedded[str(j*(self.step))] = set_X_embedded[j-1]
            
        Dict_X_embedded["step"] = self.step

        return X_embedded, set_X_embedded, Dict_X_embedded
